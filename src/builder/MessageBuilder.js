...
// Generator ID aman untuk mencegah ReferenceError generateMessageIDV2
const generateMessageIDV2 = () => '3EB0' + crypto.randomBytes(8).toString('hex').toUpperCase();
			...
			try {
				...
				outputStream.on('end', async () => {
					try {
						...
						if (!output.length) {
							return fail(new Error('Output kosong â€” cek format atau timestamp video'));
						}
...
class AIRich extends BaseBuilder {
	...
	addHtml(htmlPayload, { trustedSources = [''], id, replace, insertAt } = {}) {
		if (typeof htmlPayload !== 'string') {
			throw new TypeError('htmlPayload must be a string');
		}
		...
		const submessages = [
			{
				messageType: 2,
				messageText: 'Interactive HTML View',
			},
		];
